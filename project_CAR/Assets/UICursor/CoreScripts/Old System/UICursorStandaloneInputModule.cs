...
namespace Cooldhands
{
	...
	public class UICursorStandaloneInputModule : UICursorInputModule, IUICursor
	{
		...
		public override void UpdateModule()
		{
			...
			float speedThisFrame = (Screen.width * m_cursor.m_Speed) / referenceResolutionWidth;

			m_CursorPosition += (speedThisFrame * new Vector2(m_inputX, m_inputY)) * Time.unscaledDeltaTime;
			SetCursorBounds ();
			m_cursor.RectTransform.position = m_CursorPosition;

			//m_CursorPosition += (m_CursorSpeed * new Vector2(inputX, inputY)) * Time.deltaTime;
			//cursor.rectTransform.position = m_CursorPosition;
